...
std::string Parser::getTagValue(char const** path,
...
MLCitation Parser::parse(char* mlc_xml) {
    ...
    result.date = getTagValue(year_q, &doc);
    result.date += getTagValue(month_q, &doc);
    result.journal = getTagValue(journal_q, &doc);
